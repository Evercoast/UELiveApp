 ...
 */
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT( FMVFVoxelVertexFactoryParameters, )
...
typedef TUniformBufferRef<FMVFVoxelVertexFactoryParameters> FMVFVoxelVertexFactoryBufferRef;

/**
 * Vertex factory for point cloud rendering. This base version uses the dummy color buffer
 */
class FMVFVoxelVertexFactory : public FVertexFactory
{
	DECLARE_VERTEX_FACTORY_TYPE(FMVFVoxelVertexFactory);

public:
	FMVFVoxelVertexFactory(ERHIFeatureLevel::Type InFeatureLevel)
	...
	*/
	 ...
	 */
	void SetParameters(const FMVFVoxelVertexFactoryParameters& InUniformParameters);
