...
int32 UPicoAudioSoundWave::GeneratePCMData(uint8* PCMData, const int32 SamplesNeeded)
{
	...
	if (m_hasInitialSynced)
	{
		...
		if (SamplesAvailable > 0)
		{
			...
			if (GEngine && SamplesAvailable < SamplesNeeded)
				GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Cyan, *FString::Printf(TEXT("Audio buffer left over elements: %d PCM buffer needs: %d"), m_audioBuffer.Num(), SamplesNeeded * SampleByteSize));
		...
		{
			if (GEngine)
				GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("Zero available samples. Audio buffer starving."));
		}
	} 
	else if(!m_isReady)
	{
...
void UPicoAudioSoundWave::Sync(double timestamp)
{
	...
	while (!m_segments.empty())
	{
		...
		{
			if (GEngine)
				GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, *FString::Printf(TEXT("Discard sound during Sync(): %.3f > (%.3f + %.3f)"), timestamp, entry.timestamp, entry.duration));
